  ...
  <div>
    <cp-card class="pa-4 mt-4">
      ...
      >
        <template #top>
          <v-toolbar flat>
            <v-toolbar-title>Deflect</v-toolbar-title>
        ...
        <template #item.deflect_name="{ item }">
          <cp-col min="120">{{ item.deflect_name }}</cp-col>
        </template>
            ...
            <v-icon
              ...
              @click="
                ;(createModal.dialog = true),
                  ...
                  (createModal.deflectId = item.deflect_id),
                  (createModal.deflectName = item.deflect_name)
              "
            >
              mdi-pencil-outline
            </v-icon>
            <v-icon
              ...
              @click="
                ;(deleteModal.dialog = true),
                  (deleteModal.deflectId = item.deflect_id)
              "
            >
    ...
    >
      <v-card>
        <v-card-title>
          {{ createModal.edit ? 'แก้ไข' : 'สร้าง' }} Deflect
          <v-spacer />
          <v-btn
        ...
        <v-card-text>
          <v-form ref="formCreate" v-model="createModal.valid" lazy-validation>
            <cp-label> ชื่อ Deflect </cp-label>
            <v-text-field
              v-model="createModal.deflectName"
              :rules="createModal.deflectNameRules"
              :disabled="createModal.loading"
              outlined
              dense
              required
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Deflect', value: 'deflect_name', sortable: false },
      {
        text: 'สร้างโดย',
        value: 'created_by',
        sortable: false,
  ...
  methods: {
    ...
    async getDataList() {
      ...
      if (accessToken) {
        this.$axios
          .get(`${process.env.API_ENDPOINT}/v1/setting/deflect/`, {
            headers: {
    ...
    async createData() {
      ...
      if (accessToken) {
        ...
        this.$axios
          .post(
            `${process.env.API_ENDPOINT}/v1/setting/deflect/`,
            {
              deflect_name: this.cleanText(this.createModal.deflectName),
            },
            {
              headers: {
                Authorization: `Bearer ${accessToken}`,
          ...
          .then(({ data }) => {
            this.onNotify({
              ...
              message:
                'Deflect ' + this.createModal.deflectName + ' ถูกสร้างสำเร็จ',
            })
            this.getDataList()
          ...
          .catch((error) => {
            ...
            this.onNotify({
              ...
              message: statusCode === 409 ? 'มี Deflect นี้แล้วในระบบ' : error,
            })
          })
      }
    },
      ...
      if (accessToken) {
        ...
        this.$axios
          .put(
            `${process.env.API_ENDPOINT}/v1/setting/deflect/`,
            {
              deflect_id: this.createModal.deflectId,
              deflect_name: this.createModal.deflectName,
            },
            {
              headers: {
                Authorization: `Bearer ${accessToken}`,
    ...
    async deleteData() {
      ...
      if (accessToken) {
        ...
        this.$axios
          .delete(`${process.env.API_ENDPOINT}/v1/setting/deflect/`, {
            data: {
              deflect_id: this.deleteModal.deflectId,
            },
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
          ...
          .then(({ data }) => {
            this.onNotify({
              ...
              type: 'success',
              title: 'การดำเนินการสำเร็จ',
              message: 'Deflect ถูกลบออกจากระบบแล้ว',
            })
