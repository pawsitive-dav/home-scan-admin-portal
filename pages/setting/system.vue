...
<template>
  <div>
    <cp-card class="pa-4 mt-4">
      ...
      >
        ...
        <template #item.system_name="{ item }">
          <cp-col min="120">{{ item.system_name }}</cp-col>
        </template>
            ...
            <v-icon
              ...
              @click="
                ;(createModal.dialog = true),
                  ...
                  (createModal.systemId = item.system_id),
                  (createModal.systemName = item.system_name)
              "
            >
              mdi-pencil-outline
            </v-icon>
            <v-icon
              ...
              @click="
                ;(deleteModal.dialog = true),
                  (deleteModal.systemId = item.system_id)
              "
            >
    ...
    >
      <v-card>
        <v-card-title>
          {{ createModal.edit ? 'แก้ไข' : 'สร้าง' }} System
          <v-spacer />
          <v-btn
        ...
        <v-card-text>
          <v-form ref="formCreate" v-model="createModal.valid" lazy-validation>
            ...
            <v-text-field
              v-model="createModal.systemName"
              :rules="createModal.systemNameRules"
              :disabled="createModal.loading"
              outlined
              dense
              required
  ...
  methods: {
      ...
      if (accessToken) {
        ...
        this.$axios
          .post(
            ...
            {
              system_name: this.cleanText(this.createModal.systemName),
            },
            {
              headers: {
                Authorization: `Bearer ${accessToken}`,
          ...
          .then(({ data }) => {
            this.onNotify({
              ...
              message:
                'System ' + this.createModal.systemName + ' ถูกสร้างสำเร็จ',
            })
            this.getDataList()
      ...
      if (accessToken) {
        ...
        this.$axios
          .put(
            `${process.env.API_ENDPOINT}/v1/setting/system/`,
            {
              system_id: this.createModal.systemId,
              system_name: this.createModal.systemName,
            },
            {
              headers: {
                Authorization: `Bearer ${accessToken}`,
    ...
    async deleteData() {
      ...
      if (accessToken) {
        ...
        this.$axios
          .delete(`${process.env.API_ENDPOINT}/v1/setting/system/`, {
            data: {
              system_id: this.deleteModal.systemId,
            },
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
          ...
          .then(({ data }) => {
            this.onNotify({
              ...
              type: 'success',
              title: 'การดำเนินการสำเร็จ',
              message: 'System ถูกลบออกจากระบบแล้ว',
            })
