  ...
  <div>
    <cp-card class="pa-4 mt-4">
      ...
      >
        <template #top>
          <v-toolbar flat>
            <v-toolbar-title>Type Location</v-toolbar-title>
        ...
        <template #item.location_name="{ item }">
          <cp-col min="120">{{ item.location_name }}</cp-col>
        </template>
            ...
            <v-icon
              ...
              @click="
                ;(createModal.dialog = true),
                  ...
                  (createModal.locationId = item.location_id),
                  (createModal.locationName = item.location_name)
              "
            >
              mdi-pencil-outline
            </v-icon>
            <v-icon
              ...
              @click="
                ;(deleteModal.dialog = true),
                  (deleteModal.locationId = item.location_id)
              "
            >
    ...
    >
      <v-card>
        <v-card-title>
          {{ createModal.edit ? 'แก้ไข' : 'สร้าง' }} Location
          <v-spacer />
          <v-btn
        ...
        <v-card-text>
          <v-form ref="formCreate" v-model="createModal.valid" lazy-validation>
            ...
            <v-text-field
              v-model="createModal.locationName"
              :rules="createModal.locationNameRules"
              :disabled="createModal.loading"
              outlined
              dense
              required
  ...
  methods: {
      ...
      if (accessToken) {
        ...
        this.$axios
          .post(
            ...
            {
              location_name: this.cleanText(this.createModal.locationName),
            },
            {
              headers: {
                Authorization: `Bearer ${accessToken}`,
          ...
          .then(({ data }) => {
            this.onNotify({
              ...
              message:
                'โลเคชั่น ' + this.createModal.locationName + ' ถูกสร้างสำเร็จ',
            })
            this.getDataList()
          ...
          .catch((error) => {
            ...
            this.onNotify({
              ...
              message: statusCode === 409 ? 'มีโลเคชั่นนี้แล้วในระบบ' : error,
            })
          })
      }
    },
      ...
      if (accessToken) {
        ...
        this.$axios
          .put(
            `${process.env.API_ENDPOINT}/v1/setting/location/`,
            {
              location_id: this.createModal.locationId,
              location_name: this.createModal.locationName,
            },
            {
              headers: {
                Authorization: `Bearer ${accessToken}`,
    ...
    async deleteData() {
      ...
      if (accessToken) {
        ...
        this.$axios
          .delete(`${process.env.API_ENDPOINT}/v1/setting/location/`, {
            data: {
              location_id: this.deleteModal.locationId,
            },
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
          ...
          .then(({ data }) => {
            this.onNotify({
              ...
              type: 'success',
              title: 'การดำเนินการสำเร็จ',
              message: 'โลเคชั่นถูกลบออกจากระบบแล้ว',
            })
